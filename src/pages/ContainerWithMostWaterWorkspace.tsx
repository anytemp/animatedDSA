...
function BruteForceVisualizer() {
  ...
  const steps = [
    { id: 0, phase: 'init', left: null, right: null, waterHeight: null, width: null, area: null, maxArea: 0, codeLine: 3, explanation: 'Initialize maxArea = 0. We will check every possible pair of walls.', isNewMax: false },
    ...pairs.map((pair, idx) => {
    ...
    }).flat(),
    {
      ...
      maxArea: 49,
      codeLine: 12,
      explanation: 'All pairs checked. Return maxArea = 49.',
      isNewMax: false,
    }
  ];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8 gap-6">
          <div>
            ...
            <p className="text-gray-600">Check every possible pair (O(n²))</p>
          </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl border-2 border-gray-100 p-8">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl p-6 border-2 border-blue-200">
              ...
              <p className="text-sm text-blue-700 leading-relaxed">Two nested loops check every possible pair of walls.</p>
            </div>
    ...
    <div className="bg-gradient-to-br from-white via-blue-50/30 to-cyan-50/20 py-12 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-3xl shadow-xl border-2 border-gray-100 p-12 text-center">
          ...
          <h3 className="text-2xl font-bold text-gray-900 mb-4">No Standard Better Approach</h3>
          <p className="text-lg text-gray-700 leading-relaxed max-w-2xl mx-auto">
            For this problem, there is no widely recognized intermediate approach between brute force O(n²) and the optimal two-pointer O(n) solution.
          </p>
          <p className="text-lg text-gray-700 leading-relaxed max-w-2xl mx-auto mt-4">
            The optimal two-pointer approach is both the most efficient and the most elegant solution. Let's move directly to it!
          </p>
        </div>
      </div>
    </div>
...
export default function ContainerWithMostWaterWorkspace() {
  return (
    <WorkspaceTemplate
      problemId={10}
      ...
      approaches={{
        brute: {
          ...
          description: 'Check every possible pair of walls and calculate the area.',
          intuition: 'Use two nested loops to try all possible left and right wall positions.',
          howItWorks: 'For each left wall, try every right wall to its right. Calculate the area as waterHeight × width, where waterHeight is the shorter wall. Keep track of the maximum area.',
          tradeoff: 'Simple but very slow. For n walls, we check n(n-1)/2 pairs, which is O(n²).',
        },
        better: {
          title: 'No Standard Better Approach',
          description: 'There is no widely recognized intermediate approach between brute force and optimal.',
          intuition: 'For this problem, the optimal two-pointer approach is both the most efficient and the most elegant solution.',
          howItWorks: 'The two-pointer approach directly achieves O(n) time complexity, which is optimal for this problem.',
          tradeoff: 'Skip to the optimal approach to see the most efficient solution.',
        },
        optimal: {
          title: 'Optimal: Two-Pointer Approach',
          description: 'Use two pointers starting at the ends and moving inward.',
          intuition: 'Start with the widest container (leftmost and rightmost walls). The area is limited by the shorter wall. To potentially find a larger area, we must move the shorter wall inward, hoping to find a taller wall.',
          howItWorks: 'Initialize left = 0 and right = n-1. Calculate the area. Move the pointer pointing to the shorter wall inward. Repeat until left >= right. The key insight: moving the taller wall cannot increase the area because the width decreases and the height is still limited by the shorter wall.',
